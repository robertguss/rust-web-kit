...
| Session | `crates/core/src/auth/session_store.rs`, `extract.rs` (`CurrentUser`, `RequireAuth`) |
...
| UI | `apps/web/src/routes/{login,register,forgot-password,reset-password,verify-email}.tsx` |
...
`CurrentUser::login` calls `session.cycle_id()` then stores `user_id`. `RequireAuth` is 401 when missing. Integration tests use the cookie-aware client in `crates/api/tests/common`.
...
`/app` `beforeLoad` uses `fetchCurrentUser` (`apps/web/src/api/query.ts`). 401 → `/login?redirect=` (return-to restricted to `/app*`). Forms map 422 `errors` with `applyProblem`.

## Tests to keep green

Register → login → me → logout → me 401; duplicate email 409; bad password 401; validation 422; verify/reset happy and expired; forgot-password always 204; session cookie value changes on login; OAuth unconfigured 404 and bad state → `/login?error=`.
